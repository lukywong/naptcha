...
void CNaptcha::Generate(const FunctionCallbackInfo<Value>& args)
{
  Isolate* isolate = Isolate::GetCurrent();
  HandleScope scope(isolate);

  naptcha_object::Text = _ToString(args[0]->ToString());
  naptcha_object::FileName = _ToString(args[1]->ToString());
  naptcha_object::Count = args[2]->Int32Value();
  naptcha_object::Width = args[3]->Int32Value();
  naptcha_object::Height = args[4]->Int32Value();
  naptcha_object::Offset = args[5]->Int32Value();
  naptcha_object::Quality = args[6]->Int32Value();
  naptcha_object::IsJpeg = args[7]->Int32Value();
  naptcha_object::FontSize = args[8]->Int32Value();

  Save();

  args.GetReturnValue().Set(String::NewFromUtf8(isolate, (naptcha_object::FileName).c_str()));
}
